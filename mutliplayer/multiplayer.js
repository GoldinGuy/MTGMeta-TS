...
fs.readFile("input_json/decks-" + FORMATS[0] + ".json", "utf8", function (err, json) {
    ...
    function closestCards(a_card, b) {
        ...
        for (const unique_card of unique_cards) {
            let dist = distance(apparationRatio(unique_card.card_name)[0], a_card_app);
    ...
    function commonDecks(card_name) {
        let common_decks = [];
        for (const commander of commanders) {
            let idx = commander.cards.findIndex(c => c.card_name.includes(card_name.toString()));
            if (idx != -1) {
                let percent = Math.min((commander.cards[idx].quantity / commander.instances) * 100, 100);
    ...
    function formatCards(k) {
        ...
        for (const unique_card of cards.splice(0, k)) {
            formatCards.push({
                ...
                percentage_of_total_cards: ((unique_card.quantity / total_cards_no_basics) * 100).toFixed(2) +
                    "%",
