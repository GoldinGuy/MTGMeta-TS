...
data, k, type, max_it, fn_dist) {
    ...
    do {
        ...
        if (data[0].length > 0) {
            ...
            for (const i in data) {
                let idx = indexes[i]; // Centroid for that item
                // let v_sum: number = sum[idx]; // Sum values for this centroid
                let v_sum = sum[idx]; // Sum values for this centroid
                let vector = data[i]; // Current vector
            ...
            cent_moved = true;
            for (let j = 0; j < k; j++) {
                let cents_j = cents[j]; // Current centroid
