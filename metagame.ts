...
for (let i = 0; i < NUM_CLUSTERS; i++) {
	...
	let deck_items: Array<[Deck, number]> = decksByIdx(i);
...
for (const unique_card of unique_cards) {
	...
	if (unique_card.quantity >= unique_cards[0].quantity * CARD_CUTOFF) {
		let format_card: FormatCard = {
			...
			percentage_of_total_decks: ((unique_card.decks_in / decks.length) * 100),
			percentage_of_total_cards: ((unique_card.quantity / total_cards) * 100)
		};
